 ...
 *
 * To create a multithreaded test case, simply extend this class.
 *
 * Any method with a name that starts with "thread",
 * that has no parameters and a void return type is a thread method.
 *
 * Each thread method will be run in a seperate thread.
 *
 * <p>
 ...
 *
 * <p>
 * The method {@link TestFramework#runOnce(TestCase)} can be used to run a MultithreadedTestCase once.
 * The method {@link TestFramework#runManyTimes(TestCase, int)} can be used to run a multithread test case multiple times
 * (to see if different interleavings produce different behaviors).
 *
 * <p>
 * There are several additional methods you can use in designing test cases.
 *
 * The MultithreadedTestCase maintains a metronome or clock, and ticks off intervals.
 *
 * You can get the current tick with {@link #getTick()}
 * and you can wait until a particular tick with {@link #waitForTick(int)}.
 *
 * The metronome isn't a free running clock;
 * it only advances to the next tick when all threads are blocked or waiting.
 *
 * Also, when all threads are blocked, if at least one thread isn't waiting
 * for the metronome to advance, the system declares a deadlock to have occurred
 * and terminates the test case (unless one of the threads is in state TIMED_WAITING).
 *
 * <p>
 * You can set a command line parameter -Dtunit.trace=true
 * to cause tracing messages to be printed by the metronome frame,
 * or invoke {@link TestCase#setTrace(boolean)} to turn tracing on or off.
 *
 * <p>
 * You can set command line parameter -Dtunit.runLimit=10
 * to cause a test case to fail if at least one thread stays in a runnable state
 * for more than 10 seconds without becoming blocked or waiting for a metronome tick.
 *
 * Use different values for shorter or longer time limits.
 *
 * @see TestFramework
 *
 * @author William Pugh
...
public abstract class TestCase {
   ...
   * This clock is advanced by the clock thread started by {@link TestFramework}.
   ...
   */
  boolean trace = Boolean.getBoolean("tunit.trace");
   ...
   */
  final Map<Thread, Integer> threads = new IdentityHashMap<>();

  /**
   * ThreadLocal containing a reference to the current instance of this class for each thread.
   * When a thread completes or dies, its reference to this class is removed.
   */
  final static ThreadLocal<TestCase> currentTestCase = new ThreadLocal<>();

  /**
   * This method is called right after a new testcase thread is created by the {@link TestFramework}.
   * It provides initial values for {@link #currentTestCase} and {@link #threads}.
   */
  void hello() {
  ...
  /**
   * Map a thread name to all test case threads as they are created,
   * primarily so that they can be accessed by each other.
   *
   * @see #getThreadByName(String)
   * @see #getThread(int)
   */
  Map<String, Thread> methodThreads = new HashMap<>();

  /**
   * Get a thread given the method name that it corresponds to.
   * E.g. to get the thread running the contents of the method
   * <code>thread1()</code>, call <code>getThreadByName("thread1")</code>
   *
   * <p>
   * NOTE: {@link #setUp()} is called before threads are created,
   * so this method returns null if called from {@link #setUp()}
   * (but not from {@link #tearDown()}).
   *
   * @see #getThread(int)
   *
   ...
   * the prefix "thread" followed by an integer (represented by <code>index</code>.
   * e.g. getThread(1) returns the thread that <code>thread1()</code> is running in.
   *
   * <p>
   * NOTE: {@link #setUp()} is called before threads are created,
   * so this method returns null if called from {@link #setUp()}
   ...
   * If the method name is already associated with a Thread,
   * the old thread is returned, otherwise null is returned
   */
  ...
  /**
   * Force this thread to block until the thread metronome reaches the specified value,
   * at which point the thread is unblocked.
   *
   ...
   * It simply looks up the ordinal and adds 1 to determine the clock tick to wait for.
   *
   * @see #waitForTick(int)
   *
   ...
   * Primarily useful in assert statements.
   *
   * @see #assertTick(int)
   *
  ...
  /**
   * Read locks are acquired when clock is frozen and
   * must be released before the clock can advance in a waitForTick().
   */
   ...
   *
   * Use this to block the current thread with a time limit, but prevent the clock
   * from advancing due to a {@link #waitForTick(int)} in another thread.
   *
   * This statements that occur when clock is frozen should be
  ...
  /**
   * Unfreeze a clock that has been frozen by {@link #freezeClock()}.
   ...
   * {@link #waitOn(Object)} or {@link #awaitOn(Condition)}
   * should return immediately.
   *
   * @see #skipNextWait()
   */
  final static ThreadLocal<Boolean> skipNextWait = new ThreadLocal<>() {
    @Override
   ...
   *
   * Use this to make tests more robust.
   */
  ...
  /**
   * This method is a replacement for {@link Object#wait()}.
   *
   * It suppresses the {@link InterruptedException}
   * that you would otherwise have to deal with,
   * and allows automated skipping of the next wait.
   *
   * The method {@link #skipNextWait()} will force that thread
   * to immediately return from the next call to this method.
   *
   * Designing your tests so that they work even if {@link Object#wait()}
   * occasionally returns immediately will make your code much more robust
   * in face of several potential threading issues.
   *
  ...
  public static void waitOn(Object o) {
    ...
    var thisTestCase = currentTestCase.get();
  ...
  /**
   * This method is a replacement for {@link Condition#await()}.
   *
   * It suppresses the {@link InterruptedException}
   * that you would otherwise have to deal with,
   * and allows automated skipping of the next wait.
  ...
  /**
   * A ThreadLocal that contains a Random number generator.
   * This is used in {@link #mayYield()}
   *
   * @see #mayYield()
   */
  final static ThreadLocal<Random> mtcRandomizer = new ThreadLocal<>() {
    @Override
   ...
   * Use this between statements to generate more interleavings.
   */
